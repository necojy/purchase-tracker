      ...
      {isExpanded && (
        <div className="border-t border-gray-100 p-6 bg-gray-50/30">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="w-full">
              <label className="block text-xs font-bold text-gray-400 mb-1">購買地方</label>
              <select value={editForm.location} onChange={(e) => setEditForm({...editForm, location: e.target.value})} disabled={record.isRefunded} className="w-full border rounded-xl p-2.5 bg-white text-sm font-bold disabled:opacity-50">
                <option value="蝦皮">蝦皮</option>
                <option value="屈臣氏">屈臣氏</option>
              </select>
            </div>
            <div className="w-full">
              <label className="block text-xs font-bold text-gray-400 mb-1">付款方式</label>
              <select value={editForm.paymentMethod} onChange={(e) => setEditForm({...editForm, paymentMethod: e.target.value})} disabled={record.isRefunded} className="w-full border rounded-xl p-2.5 bg-white text-sm font-bold disabled:opacity-50">
